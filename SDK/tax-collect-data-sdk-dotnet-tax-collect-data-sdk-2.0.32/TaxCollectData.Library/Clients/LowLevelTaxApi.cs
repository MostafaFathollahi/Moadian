...
public class LowLevelTaxApi : ILowLevelTaxApi
{
    ...
    public List<InvoiceStatusInquiryResponseDto> InquiryInvoiceStatus(List<string> taxIds)
    {
        using var taskContext = new JoinableTaskContext();
        var taskFactory = new JoinableTaskFactory(taskContext);
        return taskFactory.Run(async () => await InquiryInvoiceStatusAsync(taxIds).ConfigureAwait(true));
    }

    public async Task<List<InvoiceStatusInquiryResponseDto>> InquiryInvoiceStatusAsync(List<string> taxIds)
    {
        var request = _requestProvider.GetInquiryInvoiceStatusRequest(taxIds);
        return await SendRequestAsync<List<InvoiceStatusInquiryResponseDto>>(request).ConfigureAwait(false);
    }

    public RegisterPaymentResultModel RegisterPaymentRequest(RegisterPaymentRequestDto paymentRequestDto)
    {
        using var taskContext = new JoinableTaskContext();
        var taskFactory = new JoinableTaskFactory(taskContext);
        return taskFactory.Run(async () => await RegisterPaymentRequestAsync(paymentRequestDto).ConfigureAwait(true));
    }

    public async Task<RegisterPaymentResultModel> RegisterPaymentRequestAsync(RegisterPaymentRequestDto paymentRequestDto)
    {
        var request = _requestProvider.GetRegisterPaymentRequest(paymentRequestDto);
        return await SendRequestAsync<RegisterPaymentResultModel>(request).ConfigureAwait(false);
    }
